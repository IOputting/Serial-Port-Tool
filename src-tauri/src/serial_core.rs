...
#[tauri::command]
pub fn set_dtr_rts(state: State<'_, PortState>, dtr: bool, rts: bool) -> Result<(), String> {
...
#[tauri::command]
pub fn disconnect_port(state: State<'_, PortState>) {
    state.should_read.store(false, Ordering::Relaxed);
    let mut state_writer = state.serial_writer.lock().unwrap();
    *state_writer = None;
}
...
#[tauri::command]
pub fn send_data(state: State<'_, PortState>, data: String, is_hex: bool) -> Result<(), String> {
    let mut writer_guard = state.serial_writer.lock().unwrap();
    if let Some(writer) = writer_guard.as_mut() {
...
#[tauri::command]
pub fn send_file(app_handle: tauri::AppHandle, state: State<'_, PortState>, file_path: String) -> Result<(), String> {
    let writer_opt = {
